...
// TODO should this live in evalglue or eval itself?

type ConfiguredProviders interface {
	Providers

	ProviderInstance(ctx context.Context, addr addrs.AbsProviderInstanceCorrect) (providers.Interface, tfdiags.Diagnostics)

	OpenEphemeralResourceInstance(ctx context.Context, addr addrs.AbsResourceInstance, cfgVal cty.Value, provider addrs.Provider, providerInstance *addrs.AbsProviderInstanceCorrect) (cty.Value, tfdiags.Diagnostics)

	// ConfiguredFunction constructs a cty function given a provider and a function address.
	ConfiguredFunction(ctx context.Context, addr addrs.AbsProviderInstanceCorrect, pf addrs.ProviderFunction, rng hcl.Range) (function.Function, tfdiags.Diagnostics)

	Close(ctx context.Context) tfdiags.Diagnostics
}

type configSupplier func(ctx context.Context, addr addrs.AbsProviderInstanceCorrect) (cty.Value, tfdiags.Diagnostics)

func NewConfiguredProviders(unconfigured Providers, config configSupplier) ConfiguredProviders {
...
type configuredProviders struct {
	Providers
...
func (p *configuredProviders) ProviderInstance(ctx context.Context, addr addrs.AbsProviderInstanceCorrect) (providers.Interface, tfdiags.Diagnostics) {
	p.activeMu.Lock()
...
func (p *configuredProviders) OpenEphemeralResourceInstance(ctx context.Context, addr addrs.AbsResourceInstance, cfgVal cty.Value, provider addrs.Provider, providerInstance *addrs.AbsProviderInstanceCorrect) (cty.Value, tfdiags.Diagnostics) {
...
// ConfiguredFunction constructs a cty function given a provider and a function address.
func (p *configuredProviders) ConfiguredFunction(ctx context.Context, providerInstance addrs.AbsProviderInstanceCorrect, pf addrs.ProviderFunction, rng hcl.Range) (function.Function, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
...
func (p *configuredProviders) Close(ctx context.Context) tfdiags.Diagnostics {
	var diags tfdiags.Diagnostics
