...
func (c *ConfigInstance) DrivePlanning(ctx context.Context, buildGlue func(*PlanningOracle) PlanGlue) (*PlanningResult, tfdiags.Diagnostics) {
	...
	configuredProviders := evalglue.NewConfiguredProviders(c.evalContext.Providers, func(ctx context.Context, addr addrs.AbsProviderInstanceCorrect) (cty.Value, tfdiags.Diagnostics) {
		ctx = grapheval.ContextWithNewWorker(ctx)
	...
	oracle.providers = configuredProviders
	// Inject configured providers
	evalGlue.providers = configuredProviders
...
type planningEvalGlue struct {
	...
	providers      evalglue.ConfiguredProviders
}
